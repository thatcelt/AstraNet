...
func SendMessage(c fiber.Ctx) error {
	...
	if !middleware.CheckMessageRateLimit(auid) {
...
// GetStickerCollections returns user's active sticker collections
func GetStickerCollections(c fiber.Ctx) error {
	db := middleware.GetDBFromContext(c)
	auid := middleware.GetAUIDFromContext(c)
	...
	collectionType := c.Query("type", "my-active-collection")

	var collections []chat.StickerCollection

	if collectionType == "my-active-collection" {
		// Get user's owned sticker collections
		var userCollections []chat.UserStickerCollection
		db.Where("user_id = ? AND status = 1", auid).Find(&userCollections)

		if len(userCollections) > 0 {
			collectionIds := make([]string, len(userCollections))
			for i, uc := range userCollections {
				collectionIds[i] = uc.CollectionID
			}
			db.Where("collection_id IN ?", collectionIds).
				Preload("StickerList").
				Find(&collections)
		}
	} else {
		// Get all available collections
		db.Preload("StickerList").Find(&collections)
	}

	return c.JSON(fiber.Map{
		"stickerCollectionList": collections,
	})
}

// GetCollectionStickers returns stickers in a specific collection
func GetCollectionStickers(c fiber.Ctx) error {
	db := middleware.GetDBFromContext(c)
	collectionId := c.Params("collectionId")

	var stickers []chat.Sticker
	db.Where("collection_id = ?", collectionId).Find(&stickers)
...
// GetStoreItems returns user's owned items (bubbles or frames)
func GetStoreItems(c fiber.Ctx) error {
	...
	sectionName := c.Query("sectionName")

	switch sectionName {
	case "bubbles":
		var userBubbles []chat.UserChatBubble
		db.Where("user_id = ? AND status = 1", auid).Find(&userBubbles)

		var bubbles []chat.ChatBubble
		if len(userBubbles) > 0 {
			bubbleIds := make([]string, len(userBubbles))
			for i, ub := range userBubbles {
				bubbleIds[i] = ub.BubbleID
			}
			db.Where("bubble_id IN ?", bubbleIds).Find(&bubbles)
		}
	...
	case "avatar-frames":
		var userFrames []chat.UserFrameCollection
		db.Where("user_id = ? AND status = 1", auid).Find(&userFrames)

		var frames []interface{}
		if len(userFrames) > 0 {
			frameIds := make([]string, len(userFrames))
			for i, uf := range userFrames {
				frameIds[i] = uf.FrameID
			}
			db.Table("avatar_frames").Where("frame_id IN ?", frameIds).Find(&frames)
		}
...
// ApplyBubble applies a chat bubble to a specific thread
func ApplyBubble(c fiber.Ctx) error {
	auid := middleware.GetAUIDFromContext(c)
	if auid == "" {
		return c.Status(fiber.StatusUnauthorized).JSON(response.NewError(response.InvalidRequest().StatusCode))
	}
	...
	// Validate ownership if bubbleId is provided
	if req.BubbleID != nil && *req.BubbleID != "" {
		db := middleware.GetDBFromContext(c)
		var count int64
		db.Model(&chat.UserChatBubble{}).
			Where("user_id = ? AND bubble_id = ? AND status = 1", auid, *req.BubbleID).
			Count(&count)
		if count == 0 {
			return c.Status(fiber.StatusForbidden).JSON(response.NewError(response.StatusNoPermission))
		}
