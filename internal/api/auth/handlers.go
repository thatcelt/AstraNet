...
func Login(c fiber.Ctx) error {
	...
	if middleware.IsLoginBlocked(c.IP()) {
	...
	if err != nil {
		middleware.RecordLoginFailure(c.IP())
...
func Register(c fiber.Ctx) error {
	...
	if !middleware.CheckRegisterRateLimit(c.IP()) {
		return c.Status(fiber.StatusTooManyRequests).JSON(response.NewError(response.StatusTooManyRequests))
	}
	...
	if err := c.Bind().Body(&req); err != nil {
		log.Printf("[Register] Failed to parse request body: %v, raw body: %s", err, string(c.Body()))
		return c.Status(fiber.StatusBadRequest).JSON(response.InvalidRequest())
	}

	log.Printf("[Register] Request from IP %s: email=%q, identity=%q, validationContext.identity=%q, nickname=%q, clientType=%d, type=%d, deviceID=%q, hasSecret=%v, validationContext.type=%d, code_len=%d",
		c.IP(), req.Email, req.Identity, req.ValidationContext.Identity, req.Nickname, req.ClientType, req.Type, req.DeviceID, req.Secret != "", req.ValidationContext.Type, len(req.ValidationContext.Data.Code))

	if err := req.Validate(); err != nil {
		log.Printf("[Register] Validation failed for %q: %v (email=%q, identity=%q, vc.identity=%q)", req.Email, err, req.Email, req.Identity, req.ValidationContext.Identity)
		switch err {
		case ErrInvalidEmail:
			return c.Status(fiber.StatusBadRequest).JSON(response.NewError(response.StatusInvalidEmail))
		case ErrPasswordTooShort:
			return c.Status(fiber.StatusBadRequest).JSON(response.NewError(response.StatusPasswordTooShort))
		case ErrPasswordTooLong:
			return c.Status(fiber.StatusBadRequest).JSON(response.NewError(response.StatusPasswordTooLong))
		case ErrNicknameEmpty, ErrNicknameTooShort:
			return c.Status(fiber.StatusBadRequest).JSON(response.NewError(response.StatusNicknameTooShort))
		case ErrNicknameTooLong:
			return c.Status(fiber.StatusBadRequest).JSON(response.NewError(response.StatusNicknameTooLong))
		case ErrInvalidDeviceID:
			return c.Status(fiber.StatusBadRequest).JSON(response.NewError(response.StatusInvalidDeviceID))
		case ErrInvalidCode:
			return c.Status(fiber.StatusBadRequest).JSON(response.NewError(response.StatusInvalidVerifyCode))
		default:
			return c.Status(fiber.StatusBadRequest).JSON(response.InvalidRequest())
		}
	}
...
func Validation(c fiber.Ctx) error {
	...
	// Rate limit: max 3 emails per 10 minutes per email address
	if !middleware.CheckEmailRateLimit(req.Identity) {
		return c.Status(fiber.StatusTooManyRequests).JSON(response.NewError(response.StatusEmailRateLimited))
	}

	captcha, err := captchaService.GenerateCaptcha(req.Identity)
	if err != nil {
		log.Printf("Failed to generate captcha for %s: %v", req.Identity, err)
		return c.Status(fiber.StatusInternalServerError).JSON(response.NewError(response.StatusServerError))
	}

	if err := smtpService.SendCaptchaEmail(req.Identity, captcha.Code); err != nil {
		log.Printf("Failed to send captcha email to %s: %v", req.Identity, err)
		return c.Status(fiber.StatusInternalServerError).JSON(response.NewError(response.StatusServerError))
	}

	log.Printf("Verification code sent to %s", req.Identity)
