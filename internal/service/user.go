...
func (s *UserService) UpdateUser(uid string, ndcId int, updates map[string]interface{}) (*user.User, error) {
	...
	_, err := s.GetUserByID(uid, ndcId, false)
	if err != nil {
		return nil, err
	}

	// Обновляем только профиль с конкретным uid + ndc_id
	err = s.db.Model(&user.User{}).Where("uid = ? AND ndc_id = ?", uid, ndcId).Updates(updates).Error
	if err != nil {
...
// UnfollowUserAllCommunities удаляет подписки между двумя пользователями во всех сообществах
func (s *UserService) UnfollowUserAllCommunities(uid1, uid2 string) {
	var follows []user.UserFollow
	s.db.Where(
		"(follower_uid = ? AND target_uid = ?) OR (follower_uid = ? AND target_uid = ?)",
		uid1, uid2, uid2, uid1,
	).Find(&follows)

	for _, f := range follows {
		_ = s.UnfollowUser(f.FollowerUID, f.TargetUID, f.NdcID)
	}
}
...
func (s *UserService) AddWallComment(authorUID, targetUID, content, replyTo string, ndcId int, commentType int, stickerID, stickerIcon, stickerMedia *string, mediaList *utils.MediaList) (*user.Comment, error) {
	...
	// Обновляем commentsCount у владельца стены (не у автора комментария)
	s.db.Model(&user.User{}).Where("uid = ? AND ndc_id = ?", targetUID, ndcId).UpdateColumn("comments_count", gorm.Expr("comments_count + 1"))
...
func (s *UserService) DeleteWallComment(commentId, requestorId string) error {
	...
	// NdcID берём из самого комментария — он уже знает, в каком контексте был создан
	ndcId := comment.NdcID
	...
	canDelete := role >= user.RoleCurator || // Staff (Curator+)
		comment.AuthorUID == requestorId || // Автор комментария
		comment.ParentID == requestorId // Владелец стены
	...
	// Уменьшаем счётчик комментариев у владельца стены
	s.db.Model(&user.User{}).Where("uid = ? AND ndc_id = ? AND comments_count > 0", comment.ParentID, ndcId).
...
func (s *UserService) GlobalBanUser(requesterUID, targetUID, reason string) error {
	...
	// Only Astranet Team members can global ban
	if !permSvc.IsAstranetTeam(requesterUID) {
		return ErrPermissionDenied
	}

	// Check target is not Astranet
	if permSvc.IsAstranetTeam(targetUID) {
...
func (s *UserService) GlobalUnbanUser(requesterUID, targetUID string) error {
	...
	// Only Astranet Team members can unban
	if !permSvc.IsAstranetTeam(requesterUID) {
		return ErrPermissionDenied
	}
