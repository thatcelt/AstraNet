...
// AcknowledgeNotification marks an important notification as acknowledged
func (s *NotificationService) AcknowledgeNotification(notificationID, userID string) error {
	result := s.db.Model(&notification.Notification{}).
		Where("id = ? AND recipient_uid = ? AND is_important = true", notificationID, userID).
		Update("is_acknowledged", true)

	if result.Error != nil {
		return fmt.Errorf("failed to acknowledge notification: %w", result.Error)
	}
...
// GetUnacknowledgedImportant returns unacknowledged important notifications for a user
func (s *NotificationService) GetUnacknowledgedImportant(userID string, ndcID int) ([]notification.Notification, error) {
	var notifications []notification.Notification

	query := s.db.Preload("Actor").
		Where("recipient_uid = ? AND is_important = true AND is_acknowledged = false", userID)
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get important notifications: %w", err)
	}
...
// NotifyBlogAuthorAboutLike notifies the blog author when someone likes their post
func (s *NotificationService) NotifyBlogAuthorAboutLike(blogAuthorUID, likerUID string, ndcID int, blogID, blogTitle string) error {
	// Don't notify yourself
	if blogAuthorUID == likerUID {
		return nil
	}

	var liker user.User
	if err := s.db.First(&liker, "uid = ?", likerUID).Error; err != nil {
		return fmt.Errorf("liker not found: %w", err)
	}

	n := &notification.Notification{
		ID:           generateUID(),
		RecipientUID: blogAuthorUID,
		ActorUID:     likerUID,
		Type:         notification.NotificationBlogLike,
		NdcID:        ndcID,
		ObjectID:     blogID,
		ObjectType:   notification.ObjectTypeBlog,
		Title:        blogTitle,
		Content:      fmt.Sprintf("%s liked your post", liker.Nickname),
		Icon:         liker.Icon,
	}
...
// NotifyCommentAuthorAboutLike notifies the comment author when someone likes their comment
func (s *NotificationService) NotifyCommentAuthorAboutLike(commentAuthorUID, likerUID string, ndcID int, blogID, commentID, commentContent string) error {
	// Don't notify yourself
	if commentAuthorUID == likerUID {
		return nil
	}
	...
	contentPreview := commentContent
	runes := []rune(contentPreview)
	if len(runes) > 50 {
		contentPreview = string(runes[:50]) + "..."
	}
	...
	n := &notification.Notification{
		...
		ActorUID:     likerUID,
		Type:         notification.NotificationCommentLike,
		NdcID:        ndcID,
		ObjectID:     blogID,
		ObjectType:   notification.ObjectTypeBlog,
		Title:        contentPreview,
		Content:      fmt.Sprintf("%s liked your comment", liker.Nickname),
		Icon:         liker.Icon,
	}
