...
var (
	...
	ErrNotAMember        = errors.New("not a member of this community")
)
	...
	err := s.db.Transaction(func(tx *gorm.DB) error {
		...
		com.Agent = user.DetailedAuthor{
			Author: user.Author{
				...
				IsAstranet: agentProfile.IsAstranet,
...
func (s *CommunityService) TransferAgent(ndcId int, requesterUID, newAgentUID string) error {
	return s.db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Model(&user.User{}).Where("uid = ? AND ndc_id = ?", newAgentUID, ndcId).Update("role", user.RoleAgent).Error; err != nil {
			return err
		}
		...
		return tx.Model(&community.Community{}).Where("ndc_id = ?", ndcId).Select("*").Updates(&com).Error
	})
}
...
func (s *CommunityService) JoinCommunity(ndcId int, uid string) error {
	...
	// 2. Check if profile already exists (including soft-deleted)
	var existing user.User
	err := s.db.Unscoped().Where("uid = ? AND ndc_id = ?", uid, ndcId).First(&existing).Error
	if err == nil {
		// Profile exists
		if existing.MembershipStatus == 1 && !existing.DeletedAt.Valid {
			fmt.Printf("[JoinCommunity Service] User already joined\n")
			return ErrAlreadyJoined
		}
		// Rejoin — reactivate membership and clear deleted_at
		return s.db.Transaction(func(tx *gorm.DB) error {
			if err := tx.Unscoped().Model(&user.User{}).Where("uid = ? AND ndc_id = ?", uid, ndcId).Updates(map[string]interface{}{
				"membership_status": 1,
				"deleted_at":        nil,
			}).Error; err != nil {
				return err
...
// LeaveCommunity sets membership_status to 0 so the user is no longer listed as a member
func (s *CommunityService) LeaveCommunity(ndcId int, uid string) error {
	var com community.Community
	if err := s.db.Where("ndc_id = ?", ndcId).First(&com).Error; err != nil {
		return ErrCommunityNotFound
	}
	...
	if err := s.db.Where("uid = ? AND ndc_id = ?", uid, ndcId).First(&profile).Error; err != nil {
		return ErrNotAMember
	}

	if profile.MembershipStatus == 0 {
		return ErrNotAMember
	}

	if profile.Role >= user.RoleAgent {
		return ErrPermissionDenied
	}

	return s.db.Transaction(func(tx *gorm.DB) error {
		if err := tx.Model(&user.User{}).Where("uid = ? AND ndc_id = ?", uid, ndcId).Update("membership_status", 0).Error; err != nil {
			return err
		}
		if err := tx.Model(&community.Community{}).Where("ndc_id = ?", ndcId).
			Update("members_count", gorm.Expr("MAX(members_count - 1, 0)")).Error; err != nil {
			return err
		}
		return nil
	})
...
func (s *CommunityService) GetJoinedCommunities(uid string, start, size int) ([]community.Community, error) {
	...
	err := s.db.
		Joins("JOIN users ON users.ndc_id = communities.ndc_id AND users.deleted_at IS NULL").
		Where("users.uid = ? AND users.ndc_id != 0 AND users.membership_status = 1", uid).
	...
	for i := range communities {
		communities[i].MembershipStatus = 1
	}

	return communities, nil
}

// CheckMembershipBatch sets MembershipStatus on each community for the given user.
func (s *CommunityService) CheckMembershipBatch(communities []community.Community, uid string) {
	if uid == "" || len(communities) == 0 {
		return
	}

	ndcIds := make([]int, len(communities))
	for i, c := range communities {
		ndcIds[i] = c.NdcId
	}

	var joinedNdcIds []int
	s.db.Model(&user.User{}).
		Select("ndc_id").
		Where("uid = ? AND ndc_id IN ? AND membership_status = 1", uid, ndcIds).
		Pluck("ndc_id", &joinedNdcIds)

	joinedSet := make(map[int]bool, len(joinedNdcIds))
	for _, id := range joinedNdcIds {
		joinedSet[id] = true
	}

	for i := range communities {
		if joinedSet[communities[i].NdcId] {
			communities[i].MembershipStatus = 1
		}
	}
}

// CheckMembershipSingle sets MembershipStatus on a single community for the given user.
func (s *CommunityService) CheckMembershipSingle(com *community.Community, uid string) {
	if uid == "" || com == nil {
		return
	}
	var count int64
	s.db.Model(&user.User{}).Where("uid = ? AND ndc_id = ? AND membership_status = 1", uid, com.NdcId).Count(&count)
	if count > 0 {
		com.MembershipStatus = 1
	}
}
...
func (s *CommunityService) GetCommunityMembers(ndcId int, memberType string, start, size int) ([]user.User, error) {
	...
	err := query.
		Order("role DESC, created_time DESC").
		Preload("AvatarFrame").
