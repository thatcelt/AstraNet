...
func TestUserService_Following(t *testing.T) {
	...
	// Подписка (глобальный контекст, ndcId=0)
	err := userService.FollowUser(follower.UID, target.UID, 0)
	assert.NoError(t, err)

	// Проверяем, что GetFollowing возвращает target
	following, err := userService.GetFollowing(follower.UID, 0, 0, 10)
	assert.NoError(t, err)
	...
	err = userService.UnfollowUser(follower.UID, target.UID, 0)
	assert.NoError(t, err)
	following, err = userService.GetFollowing(follower.UID, 0, 0, 10)
	assert.NoError(t, err)
