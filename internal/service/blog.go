...
func (s *BlogService) CreateBlog(uid string, ndcId int, title, content string, mediaList []utils.MediaItem, backgroundMediaList []utils.MediaItem) (*blog.Blog, error) {
	...
	// Обновляем blogsCount у автора
	s.db.Model(&user.User{}).Where("uid = ? AND ndc_id = ?", uid, ndcId).UpdateColumn("blogs_count", gorm.Expr("blogs_count + 1"))
...
func (s *BlogService) DeleteBlog(blogID string, uid string) error {
	...
	// Декремент blogsCount у автора
	s.db.Model(&user.User{}).Where("uid = ? AND ndc_id = ? AND blogs_count > 0", b.UID, b.NdcID).UpdateColumn("blogs_count", gorm.Expr("blogs_count - 1"))
	return nil
}
...
func (s *BlogService) AddBlogComment(authorUID, blogID, content, replyTo string, commentType int, stickerID, stickerIcon, stickerMedia *string, mediaList *utils.MediaList) (*user.Comment, error) {
	var b blog.Blog
...
// VoteComment - поставить или убрать лайк на комментарий
func (s *BlogService) VoteComment(commentID string, uid string, voteValue int) error {
	...
	return s.db.Transaction(func(tx *gorm.DB) error {
		var existingVote blog.CommentVote
		err := tx.Where("comment_id = ? AND user_uid = ?", commentID, uid).First(&existingVote).Error
		...
		if err == nil {
			...
			if voteValue == 0 {
				...
				if err := tx.Model(&user.Comment{}).Where("id = ?", commentID).
...
// GetUserCommentVote - получить голос пользователя за комментарий
func (s *BlogService) GetUserCommentVote(commentID string, uid string) (int, error) {
	var vote blog.CommentVote
	err := s.db.Where("comment_id = ? AND user_uid = ?", commentID, uid).First(&vote).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return 0, nil
		}
...
// fillCommentVotedValues - заполнить VotedValue для списка комментариев
func (s *BlogService) fillCommentVotedValues(comments []user.Comment, uid string) {
	if len(comments) == 0 || uid == "" {
		return
	}
	...
	var votes []blog.CommentVote
	s.db.Where("comment_id IN ? AND user_uid = ?", commentIDs, uid).Find(&votes)
	...
	for i := range comments {
		if voteType, ok := voteMap[comments[i].ID]; ok {
			comments[i].VotedValue = &voteType
		}
	}
}

// GetCommentByID - получить комментарий по ID
func (s *BlogService) GetCommentByID(commentID string) (*user.Comment, error) {
	var comment user.Comment
	if err := s.db.First(&comment, "id = ?", commentID).Error; err != nil {
		return nil, err
	}
...
func (s *BlogService) GetBlogComments(blogID, sort string, start, size int, requestorUID string) ([]user.Comment, error) {
	...
	if requestorUID != "" {
		s.fillCommentVotedValues(comments, requestorUID)
	}
...
func (s *BlogService) GetCommentReplies(commentID, sort string, start, size int, requestorUID string) ([]user.Comment, error) {
	...
	if requestorUID != "" {
		s.fillCommentVotedValues(replies, requestorUID)
	}
